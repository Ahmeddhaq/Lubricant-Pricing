    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div className="kpi-card profit">
          ...
          <p className="kpi-value">${stats.totalProfit.toLocaleString()}</p>
        </div>

        <div className="kpi-card margin">
          <p className="kpi-label">Profit Margin</p>
          <p className="kpi-value">{stats.profitMargin.toFixed(1)}%</p>
        </div>

        <div className="kpi-card avgmargin">
          <p className="kpi-label">Avg Margin</p>
          <p className="kpi-value">{stats.avgMargin.toFixed(1)}%</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="table-container">
          <h2 className="px-6 pt-6 text-xl font-bold mb-4">Top Performing SKUs</h2>
          {stats.topSkus.length === 0 ? (
            <p className="px-6 pb-6 text-gray-500">No data available</p>
          ) : (
            <div className="px-6 pb-6 space-y-3">
              {stats.topSkus.map((sku, idx) => (
                <div key={idx} className="bg-white p-4 rounded-lg border-l-4 border-green-500 hover:shadow-md transition-all">
        ...
        {/* Cost Breakdown */}
        <div className="table-container">
          <h2 className="px-6 pt-6 text-xl font-bold mb-4">Financial Summary</h2>
      ...
      {/* Recent Quotes Table */}
      <div className="table-container">
        <h2 className="px-6 pt-6 text-xl font-bold">Recent Quotes</h2>
        {stats.recentQuotes.length === 0 ? (
          <p className="px-6 py-4 text-gray-500">No quotes available</p>
        ) : (
